...
export function App() {
  ...
  function enqueueAvailableTranscriptionChunks(forceFinal: boolean) {
    ...
    const chunkSamples = Math.floor(sampleRate * TRANSCRIPTION_CHUNK_SECONDS);
    const minSamples = Math.floor(sampleRate * (forceFinal ? 0.5 : TRANSCRIPTION_CHUNK_SECONDS));
    while (samplesRef.current.length - nextChunkStartSampleRef.current >= minSamples) {
      const startSample = nextChunkStartSampleRef.current;
      const endSample = forceFinal
        ? samplesRef.current.length
        : Math.min(samplesRef.current.length, startSample + chunkSamples);
      if (!forceFinal && endSample - startSample < chunkSamples) {
        return;
      }
